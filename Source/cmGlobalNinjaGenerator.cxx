                                      ...
                                      cmOutputConverter::OutputFormat format)
{
  ...
  if (this->HasOutputPathPrefix())
    {
    convPath = ng->Convert(path, cmOutputConverter::FULL, format);
    std::string absBuildDir
      = this->GetCMakeInstance()->GetHomeOutputDirectory();
    EnsureTrailingSlash(absBuildDir);
    if (cmStartsWith(convPath, absBuildDir))
      {
      convPath = convPath.substr(absBuildDir.size());
      convPath = this->NinjaOutputPath(convPath);
      }
    }
  else
    {
    convPath = ng->Convert(path, cmOutputConverter::HOME_OUTPUT);
    }
#ifdef _WIN32
...
void cmGlobalNinjaGenerator::WriteTargetRebuildManifest(std::ostream& os)
{
  ...
  std::string ninjaBuildFile = this->NinjaOutputPath(NINJA_BUILD_FILE);
  this->WriteBuild(os,
...
std::string cmGlobalNinjaGenerator::NinjaOutputPath(const std::string& path)
{
